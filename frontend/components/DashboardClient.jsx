...
import {Multiplantas,PCP,OEE,Capacidade,Materiais,Logistica,Financas,CentralDados} from './OperationalScreens'
...
export default function DashboardClient({screen}){
 ...
 const [plants,setPlants]=useState([]),[months,setMonths]=useState([])
 const [plant,setPlant]=useState('Planta Campinas'),[start,setStart]=useState(''),[end,setEnd]=useState('')
 const [data,setData]=useState(null),[err,setErr]=useState(''),[loading,setLoading]=useState(enabled)
 const [reloadKey,setReloadKey]=useState(0),[detail,setDetail]=useState(null)
 useEffect(()=>{try{const stored=sessionStorage.getItem('ip_active_plant');if(stored)setPlant(stored)}catch(e){}},[])
 function updatePlant(v){setPlant(v);try{sessionStorage.setItem('ip_active_plant',v)}catch(e){}}
 useEffect(()=>{
   if(!enabled){setLoading(false);setData(null);return}
   let active=true;setLoading(true);setErr('')
   const q=new URLSearchParams({plant});if(start)q.set('start',start);if(end)q.set('end',end)
   getJSON(`/api/bootstrap/${screen}?${q.toString()}`).then(x=>{
      if(!active)return
      setPlants(x.plants||[]);setMonths(x.months||[])
      if(x.plant&&x.plant!==plant)updatePlant(x.plant)
      setData(x.data||{})
   }).catch(e=>{if(active){setErr(String(e));setData(null)}}).finally(()=>{if(active)setLoading(false)})
   return()=>{active=false}
 },[screen,plant,start,end,reloadKey,enabled])
 function openDrill(dimension='',value=''){
   if(!enabled)return
   const q=new URLSearchParams({plant,dimension:String(dimension||''),value:String(value||''),limit:'75'})
   if(start)q.set('start',start);if(end)q.set('end',end)
   setDetail({loading:true,dimension,value})
   getJSON(`/api/drilldown/${screen}?${q.toString()}`).then(x=>setDetail({loading:false,...x})).catch(e=>setDetail({loading:false,error:String(e)}))
 }
 ...
 return <div className="app-shell"><Sidebar/><main className="main"><div className="page">
  <PageHeader title={title} subtitle={subtitle} plant={enabled&&screen!=='multiplantas'&&screen!=='central-dados'?plant:undefined} setPlant={updatePlant} plants={plants} months={months} start={start} setStart={setStart} end={end} setEnd={setEnd}/>
  {!enabled?<EmptyState title={`${title} — no radar`} text="A tela está no escopo aprovado, mas ainda não possui contrato funcional completo. Não será exibida como se estivesse pronta."/>:
   loading?<div className="empty-state"><h2>Carregando dados…</h2><p>Consultando a base ativa e o motor determinístico.</p></div>:
   ...
   <Screen screen={screen} data={data||{}} plant={plant} start={start} end={end} onDrill={openDrill} onPlant={updatePlant}/>
  }
  <div className="footer-note">Industrial Performance v1.0.4 • conteúdo e análise em validação • base: {data?.source_base||'—'} • sem números sintéticos</div>
 </div></main>{detail&&<div className="drawer-scrim" onClick={()=>setDetail(null)}><aside role="dialog" aria-modal="true" aria-label="Aprofundamento dos registros" className="drill-drawer" onClick={e=>e.stopPropagation()}><header><div><h2>Drill-down: registros de origem</h2><p>{detail.source||'Base ativa'} · {plant} · {detail.dimension?`${detail.dimension}: ${detail.value}`:'todos os registros'}</p></div><button className="control-btn" onClick={()=>setDetail(null)}>Fechar ×</button></header>{detail.loading?<p>Carregando registros…</p>:detail.error?<p className="bad-text">{detail.error}</p>:<><div className="notice">{detail.total_rows} registros encontrados. {detail.truncated?'Exibidos os primeiros 75; refine o filtro.':'Amostra exibida integralmente.'} Fonte: {detail.source}. Não são dados inferidos.</div><div className="drawer-table"><DataTable rows={detail.rows||[]} columns={Object.keys(detail.rows?.[0]||{}).map(key=>({key,label:key}))}/></div></>}</aside></div>}</div>
}

function Screen({screen,data,plant,start,end,onDrill,onPlant}){
 const map={cockpit:Cockpit,diagnostico:Diagnostico,alavancas:Alavancas,multiplantas:Multiplantas,pcp:PCP,oee:OEE,capacidade:Capacidade,materiais:Materiais,logistica:Logistica,financas:Financas,'central-dados':CentralDados}
 const C=map[screen]
 return C?<C data={data} plant={plant} start={start} end={end} onDrill={onDrill} onPlant={onPlant}/>:<EmptyState title={`${meta[screen]?.[0]||screen} — no radar`}/>
}
...
function Cockpit({data}){
 ...
 <div className="grid grid-12">
  ...
  <Panel className="span-3" title="Onde está o dinheiro?" subtitle="Impacto potencial dos principais desvios." tag="EXPOSIÇÃO" icon="▮"><HorizontalBars data={opp} dataKey="impact" nameKey="name"/><div className="total-strip"><span>Exposição direta identificado</span><strong>{fmtMoney(data.money_total)}</strong></div></Panel>
...
function Diagnostico({data}){
 ...
 const matrix=[] // Esforço por ação ainda não validado: não posicionar pontos fictícios.
 return <>
 <div className="kpi-grid cols-6"><KpiCard label="Problemas identificados" value={fmtNumber(c.problems)} foot="6 frentes avaliadas" icon="⌕"/><KpiCard label="Exposição financeira rastreável" value={fmtMoney(c.impact)} foot="somente impactos monetizados sem dupla contagem" state="bad" icon="!"/><KpiCard label="Potencial de captura" value={fmtMoney(c.potential)} foot="A VALIDAR · captação por ação" state="good" icon="↗"/><KpiCard label="Quick-wins" value={fmtMoney(c.quick_value)} foot="A VALIDAR · prazo por ação" icon="◎"/><KpiCard label="Recomendações em análise" value={fmtNumber(c.actions)} foot="validar responsáveis e evidência" icon="◆"/><KpiCard label="Payback médio" value={c.payback_months==null?'N/D':`${fmtNumber(c.payback_months,1)} meses`} foot="requer investimento por ação" icon="▦"/></div>
 <div className="grid grid-12">
  <Panel className="span-6" title="Resumo por frente" subtitle="Exposição direta e risco separados para preservar a lógica financeira." tag="6 FRENTES" icon="▤"><DataTable rows={data.fronts||[]} columns={[{key:'front',label:'Frente'},{key:'problems',label:'Problemas'},{key:'impact',label:'Exposição direta',render:v=>fmtMoney(v)},{key:'risk',label:'Receita em risco',render:v=>fmtMoney(v)},{key:'potential',label:'Potencial',render:v=>fmtMoney(v)}]}/></Panel>
...
function Alavancas({data,plant,start,end}){
 ...
 useEffect(()=>{if(!Object.keys(targets).length)return; const t=setTimeout(()=>{setSimErr('');getJSON('/api/simulate',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({plant,targets,start,end})}).then(setSim).catch(e=>setSimErr(String(e)))},220); return()=>clearTimeout(t)},[JSON.stringify(targets),plant,start,end])
 ...
 const leverDefs=[['volume','Volume vendido','un'],['price','Preço médio','R$/un'],['mix','Mix de produtos','índice'],['mp_price','Preço de MP','R$/kg'],['mp_consumption','Consumo MP sem refugo','kg/un'],['freight_unit','Frete / unidade','R$/un'],['contracts','Contratos / serviços','R$'],['fixed','Custo fixo','R$']]
 ...
 <div className="grid grid-12">
  <Panel className="span-8" title="Principais alavancas editáveis" subtitle="Ajuste Atual → Meta; o impacto é recalculado na DRE e no EBITDA Gerencial." tag="SEM DUPLA CONTAGEM" icon="◆"><div style={{display:'flex',gap:8,marginBottom:10}}><button className="control-btn" disabled title="Premissas de cenário sugerido dependem de validação">Cenário sugerido — A VALIDAR</button><button className="control-btn" onClick={()=>setTargets(cur)}>Resetar base</button></div><div className="lever-grid">{leverDefs.map(([key,label,unit])=><Lever key={key} k={key} label={label} unit={unit} current={cur[key]} target={targets[key]} setTargets={setTargets}/>)}</div><div className="data-note">Perdas de material são derivadas do consumo específico para não criar uma segunda alavanca financeira sobre a mesma causa. Atual {fmtPct(lossNow)} → Meta derivada {fmtPct(lossTarget)}.</div></Panel>
